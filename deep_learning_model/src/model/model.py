...
def _lstm_cells(layers):
  return [tf.nn.rnn_cell.BasicLSTMCell(memory_size, state_is_tuple=True) for memory_size in layers]
    
def _recurrent_layers(layers, input_data, n_steps, reuse=False):
  scope = tf.get_variable_scope()
  if reuse:
    scope.reuse_variables()
  lstm_layers = tf.nn.rnn_cell.MultiRNNCell(_lstm_cells(layers), state_is_tuple=True)
  _x = tf.split(0, n_steps, input_data)
  rnn_output, rnn_layers = tf.nn.rnn(lstm_layers, _x, dtype=tf.float32)
  return rnn_output, rnn_layers
...
#       scope.reuse_variables()
  ...
  with tf.variable_scope("prediction_scope") as scope: 
    ...
    weights_hidden_7, biases_hidden_7 = __get_variable__(7, 2048, 1024)
  ...
  lstm_1, rnn_layers = _recurrent_layers([1024], hidden_7, n_steps = 1, reuse=reuse)
  prediction = tf.add(tf.matmul(lstm_1[-1], weights_out), biases_out, name=output_name)
