...
class TrainingWrapper():
  ...
  def run(self):
    ...
    with tf.Graph().as_default():
      ...
      prediction, weights_node, biases_node = model.inference(data_batch, keep_prob_placeholder, self.args.batch_size,
      ...
      eval_prediction, _, _ = model.inference(eval_data_placeholder, keep_prob_placeholder,
      ...
      train_loss_lin = tf.summary.scalar('loss_linear_x', loss_split[0])
      train_loss_ang = tf.summary.scalar('loss_angular_yaw', loss_split[1])
      train_learning_rate = tf.summary.scalar('learning_rate', learning_rate)

      eval_loss = tf.summary.scalar('loss', evaluation)
      eval_loss_lin = tf.summary.scalar('loss_linear_x', evaluation_split[0])
      eval_loss_ang = tf.summary.scalar('loss_angular_yaw', evaluation_split[1])

      summary_op = tf.summary.merge([train_loss, train_loss_lin, train_loss_ang,
        train_learning_rate])
      ...
      summary_writer = tf.summary.FileWriter(logdir=os.path.join(storage_path, 'train'), graph=self.sess.graph)
      eval_summary_writer = tf.summary.FileWriter(logdir=os.path.join(storage_path, 'eval'), graph=self.sess.graph)
