...
SafetyModule::SafetyModule(double robotRadius, double minimumDistanceToRobot, double timeOffset,
                           double maximumDeceleration)
    : _robotRadius(robotRadius), _minimumDistanceToRobot(minimumDistanceToRobot),
      _timeOffset(timeOffset), _maxDeceleration(maximumDeceleration)
{
}

bool SafetyModule::motionIsSafe(const Eigen::Vector2d& currentPosition,
                                ...
                                const std::vector<Eigen::Vector2d>& pointsLaser)
{
  ...
  if (_motionIsSafe) {
    ROS_DEBUG_STREAM("distStop: " << distStop << ", transVel: " << transVel
                                  << ", rotVel: " << rotVel);
    if (abs(rotVel) <= _rotVelThreshold) {
      for (const Eigen::Vector2d& laserMeas : pointsLaser) {
        if (transVel > 0) {
          collision =
              (std::abs(laserMeas(1)) < _robotRadius + _minimumLateralDistance) &&
              (laserMeas(0) < distStop + _distSensorToEdge + _minimumLongitudinalDistance) &&
              (laserMeas(0) > positionRobotStart(1));
          if (collision)
            ROS_DEBUG_STREAM_NAMED(
                "safety_module",
                "Anticipated collision (straight forward driving). Stopping distance is "
                    << distStop);
        } else if (transVel < 0) {
          collision =
              (std::abs(laserMeas(1)) < _robotRadius + _minimumLateralDistance) &&
          ...
          if (collision)
            ROS_DEBUG_STREAM_NAMED(
                "safety_module",
                "Anticipated collision (straight backward driving). Stopping distance is "
                    << distStop);
        }
      ...
      if (radiusRobotTrajectory > _robotRadius) {
        positionCenter = computeRotationCenterOfTrajectory(
            radiusRobotTrajectory, transVel, rotVel, positionRobotStart, currentHeadingVector);
        positionRobotEnd =
            computeStoppingPoint(transVel, rotVel, positionRobotStart, positionCenter);

        // generate hyperplanes that form the limits of the critical collision region
        auto hyperCenterStart =
            Eigen::Hyperplane<double, 2>::Through(positionCenter, positionRobotStart);
        auto hyperCenterEnd =
            Eigen::Hyperplane<double, 2>::Through(positionCenter, positionRobotEnd);

        // find sign of the distance (critical region on which side of the halfspace) with respect
        // to the halfspaces
        ...
        for (const Eigen::Vector2d& laserMeas : pointsLaser) {
          ...
          collision =
              (distCenterSquared <=
               (radiusRobotTrajectory + _robotRadius + _minimumLateralDistance) *
                   (radiusRobotTrajectory + _robotRadius +
                    _minimumLateralDistance)) &&  // within sector of circle
              (distCenterSquared >=
               (radiusRobotTrajectory - _robotRadius - _minimumLateralDistance) *
                   (radiusRobotTrajectory - _robotRadius - _minimumLateralDistance)) &&
              (signForCollisionCenterStart ==
               getSignOfDistance(hyperCenterStart, laserMeas)) &&  // within halfspace limits
    ...
    if (!_motionIsSafe)
      os << "Collision with command tv: " << transVel << ", rv: " << rotVel
         << ", brake dist: " << distStop << ". ";
  }
...
Eigen::Vector2d SafetyModule::transformPointPolar2KarthesianCoordinates(const double range,
                                                                        const double alpha)
{
...
int SafetyModule::getSignOfDistance(const Eigen::Hyperplane<double, 2>& hyperplane,
...
{
  double dTheta = (computeStoppingDistance(transVel) + _minimumLongitudinalDistance) /
                  abs(transVel / rotVel) * sign(rotVel);
  Eigen::Rotation2D<double> rotationMatrix(dTheta);
  return positionCenter + rotationMatrix * (positionStart - positionCenter);
}

Eigen::Vector2d SafetyModule::computeRotationCenterOfTrajectory(
    const double radius, const double transVel, const double rotVel,
    const Eigen::Vector2d& positionStart, const Eigen::Vector2d& currentHeading)
{
  Eigen::Vector2d rotatedHeading90Deg(-currentHeading(1), currentHeading(0));
  return positionStart +
         sign(rotVel) * sign(transVel) * radius *
             rotatedHeading90Deg;  // center on the left if positive rotation rate
}
...
bool SafetyModule::objectTooClose(const Eigen::Vector2d& laserMeas,
                                  const Eigen::Vector2d& positionRobot,
                                  double& laserPointDistToCenterSquared)
{
  laserPointDistToCenterSquared = (laserMeas - positionRobot).squaredNorm();
  bool tooCloseToRobotCenter =
      (laserPointDistToCenterSquared <=
       (_robotRadius + _minimumDistanceToRobot) * (_robotRadius + _minimumDistanceToRobot));
  bool tooCloseToSensor(laserMeas.norm() <= _minimumDistanceToSensor);
  ROS_DEBUG_STREAM_COND_NAMED(tooCloseToSensor || tooCloseToRobotCenter, "safety_module",
                              "Distance to robot center is "
                                  << sqrt(laserPointDistToCenterSquared));
  ROS_DEBUG_STREAM_COND_NAMED(tooCloseToSensor || tooCloseToRobotCenter, "safety_module",
                              "Distance to sensor is " << laserMeas.norm());
  ROS_DEBUG_STREAM_COND_NAMED(tooCloseToSensor || tooCloseToRobotCenter, "safety_module",
                              "Object is too close to sensor: " << tooCloseToSensor
                                                                << ", too close to robot center: "
                                                                << tooCloseToRobotCenter);
